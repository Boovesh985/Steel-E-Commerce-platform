          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-end gap-2">
              ...
              {phoneVerified || (phoneUnchanged && user?.phoneVerified) ? (
                ...
                >
                  {otpSent ? (otpCountdown > 0 ? `Resend (${otpCountdown}s)` : 'Resend') : 'Verify'}
                </Button>
              ) : (
                <div className="flex items-center gap-1 h-11 px-3 text-red-500">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01M12 3a9 9 0 110 18 9 9 0 010-18z" />
                  </svg>
                  <span className="text-body-sm font-medium whitespace-nowrap">Not verified</span>
                </div>
              )}
            </div>
            {!profileForm.phone && (
